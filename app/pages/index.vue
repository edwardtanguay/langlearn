...
watch(isFlipped, (newVal) => {
  if (newVal) {
    ...
    // Optionally clear prefetch if flipped back, but keeping it is fine.
  }
})
...
function handleSearch() {
  ...
  isSearchPending.value = true // Set immediately to prevent "0 results" flash
  ...
  // 1. Fetch results quickly so they appear immediately
  ...
  // 2. Require 2 seconds of inactivity before dropping the "searching..." header
...
function markAction(actionTaken: string, newStatus?: string) {
  ...
  // Optimistically update card stats message immediately
  if (cardStats.value) {
...
// Add a new tag to the current card (creates tag globally if needed)
function submitNewTag() {
  ...
  if (!currentTags.includes(val)) {
    ...
    // Optimistically add to allTags so it appears immediately
...
async function handleSelectCard(card: Flashcard) {
  try {
    ...
    // Add to front of test queue (or move to front if already there)
    testQueue.value = [copyResult, ...testQueue.value.filter(c => c.id !== copyResult.id)]
    currentQueueIndex.value = 0
    isFlipped.value = false
    sliderValue.value = copyResult.rank
    
    // Clear search mode, but keep query so user can return
    isSearching.value = false
    isSearchPending.value = false
  } catch (err) {
...
function handleGlobalKeyDown(event: KeyboardEvent) {
  ...
  // If back card and CTRL-ENTER pressed, edit the card
...
/* Card entry/exit slide animations */
...
/* Content ↔ edit-form crossfade on card back */
...
/* Layout transition crossfade (Melt like butter, but fast) */
